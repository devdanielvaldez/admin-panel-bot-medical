...
const WorkDayForm = () => {
  ...
  useEffect(() => {
    const fetchWorkDayById = async () => {
      if (id) {
        try {
          const response = await axios.get(`https://api-jennifer-wkeor.ondigitalocean.app/api/available-work-days/${id}`);
    ...
    try {
      ...
      if (id) {
        ...
        response = await axios.put(
          `https://api-jennifer-wkeor.ondigitalocean.app/api/available-work-days/edit/${id}`,
          { dayOfWeek, workHours },
          { headers: { branchid } }
        );
      } else {
