...
const WorkDayTable = () => {
  ...
  const fetchWorkDays = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://dra-daines-uduu3.ondigitalocean.app/api/available-work-days/list");
  ...
  const toggleWorkDayStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      const response = await fetch(`https://dra-daines-uduu3.ondigitalocean.app/api/available-work-days/toggle/${id}`, {
        method: "PUT",
        headers: {
