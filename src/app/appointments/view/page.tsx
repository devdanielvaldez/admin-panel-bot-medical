...
const AppointmentTable = () => {
  ...
  const InsuranceModal = ({ ap }: { ap: Appointment }) => {
    ...
    const handleAuthorize = async () => {
      ...
      setIsLoading(true);
      await axios
        .post('https://api-jennifer-wkeor.ondigitalocean.app/api/insurances/autorizar/humano', body)
        .then(async (data) => {
          ...
          await axios
            .put(`https://api-jennifer-wkeor.ondigitalocean.app/api/appointments/save-authorization/${data?.data?.sessionClose?.autorizacion?.numeroAutorizacion}/${ap.appointmentId}/${data.data.montoArs.montoArs}`, {})
            .then((res) => {
              setIsLoading(false);
              setShowInsuranceModal(false);
  ...
  const fetchAppointments = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://api-jennifer-wkeor.ondigitalocean.app/api/appointments/all", {
        headers: {
